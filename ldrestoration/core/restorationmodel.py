...
from ldrestoration.utils.networkalgorithms import (network_cycles_basis,
...
class RestorationModel:
    ...
    def constraints_base(self,
        ...
        @timethis
        def voltage_limits_rule_base(vmax_sq: float,
                                     vmin_sq: float) -> None:
        ...
        # generate constraints for network connectivity
        voltage_limits_rule_base(vmax_sq = vmax ** 2,
                                 vmin_sq = vmin ** 2)
                ...
                if 'a' in each_line['phases']:
                    ...
                    if each_line['is_switch']:
                        ...
                        self.model.voltage_balance.add(self.model.Via[source_node_idx] - self.model.Via[target_node_index] - 
                                                    ...
                                                    (x_ab + np.sqrt(3) * r_ab) / (baseZ * KW_TO_MW_FACTOR) * self.model.Qijb[edge_index] + 
                                                    (r_ac + np.sqrt(3) * x_ac) / (baseZ * KW_TO_MW_FACTOR) * self.model.Pijc[edge_index] + 
                                                    (x_ac - np.sqrt(3) * r_ac) / (baseZ * KW_TO_MW_FACTOR) * self.model.Qijc[edge_index] == 0
                                                    )
                ...
                if 'b' in each_line['phases']:
                    ...
                    if each_line['is_switch']:
                        ...
                        self.model.voltage_balance.add(self.model.Vib[source_node_idx] - self.model.Vib[target_node_index] - 
                                                    ...
                                                    (x_ba - np.sqrt(3) * r_ba) / (baseZ * KW_TO_MW_FACTOR) * self.model.Qija[edge_index] + 
                                                    (r_bc - np.sqrt(3) * x_bc) / (baseZ * KW_TO_MW_FACTOR) * self.model.Pijc[edge_index] + 
                                                    (x_bc + np.sqrt(3) * r_bc) / (baseZ * KW_TO_MW_FACTOR) * self.model.Qijc[edge_index] == 0
                                                    )
                ...
                if 'c' in each_line['phases']:
                    ...
                    if each_line['is_switch']:
                        ...
                        self.model.voltage_balance.add(self.model.Vic[source_node_idx] - self.model.Vic[target_node_index] - 
                                                    ...
                                                    (x_ca + np.sqrt(3) * r_ca) / (baseZ * KW_TO_MW_FACTOR) * self.model.Qija[edge_index] + 
                                                    (r_cb + np.sqrt(3) * x_cb) / (baseZ * KW_TO_MW_FACTOR) * self.model.Pijb[edge_index] + 
                                                    (x_cb - np.sqrt(3) * r_cb) / (baseZ * KW_TO_MW_FACTOR) * self.model.Qijb[edge_index] == 0
                                                    )
        ...
        def substation_positiveflow_rule_base() -> None:
            ...
            logger.info(f"Successfully added substation positive flow constraint as {self.model.substation_positive_flow}")
        ...
        def der_limit_rule_base() -> None:
            ...
            for _, each_row in self.model.DERs.iterrows():
                ...
                edge_index = self.edge_indices_in_tree[(self.circuit_data['substation'], 
                                                        each_row['connected_bus'])]                
    ...
    def objective_load_only(self) -> None:
        ...
        self.model.restoration_objective = Objective(expr=(sum(self.model.si[i] * self.model.active_demand_each_node[self.demand_node_indices_in_tree[i]] for i in self.model.v_i)), sense=maximize)          
    
    @timethis
    def objective_load_and_switching(self,
                                     alpha: float = 1.0,
                                     beta: float = 0.2) -> None:
        """Objective to minimize the loss of load or maximize the total load pick up
        ...
        self.model.restoration_objective = Objective(expr=(alpha * sum(self.model.si[i] * self.model.active_demand_each_node[self.demand_node_indices_in_tree[i]] for i in self.model.v_i) + 
                                                           ...
                                                           beta * sum(self.model.xij[k] for k in self.model.tie_switch_indices)), sense=maximize)  
        
    @timethis
    def objective_load_switching_and_der(self,
                                         alpha: float = 1.0,
                                         beta: float = 0.2,
        ...
        self.model.restoration_objective = Objective(expr=(alpha * sum(self.model.si[i] * self.model.active_demand_each_node[self.demand_node_indices_in_tree[i]] for i in self.model.v_i) + 
                                                           ...
                                                           gamma * sum(self.model.xij[l] for l in self.model.virtual_switch_indices)), sense=maximize)
    ...
    def solve_model(self,
        ...
        if solver == 'gurobi':    
            ...
            optimization_solver = SolverFactory(solver, solver_options=solver_options, **kwargs)
        ...
        if save_results:
            self.save_variable_results(results, 
                                       results_filename=results_filename)         
        
        return self.model, results
    
    @timethis
    def save_variable_results(self,
