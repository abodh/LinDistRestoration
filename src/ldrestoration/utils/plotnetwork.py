...
def lat_lon_validator(lat: float, lon: float) -> None:
    """
    ...
    if not (-90 <= lat <= 90 and -180 <= lon <= 180):
        raise ValueError(
            "Latitude must be between -90 and 90 degrees, and "
...
def plot_solution_map(
    ...
    **kwargs,
    ...
    lat_lon_validator(
        network_tree.nodes[next(iter(network_tree.nodes))]["lat"],
        network_tree.nodes[next(iter(network_tree.nodes))]["lon"],
    )

    if background not in ["white", "map", None]:
        raise ValueError(
            "background can either be white, map, or None (defaults to map). Please provide a valid background."
        )
    ...
    powerflow = {
        index: abs(
            round(
                model.Pija[edge].value + model.Pijb[edge].value + model.Pijc[edge].value,
                2,
            )
        )
        for index, edge in enumerate(model.x_ij)
    }
        ...
        if edge_index in model.virtual_switch_indices:
            if from_node == "sourcebus":
                ...
                Circle(
                    ...
                    opacity=0.7,
                    popup=popup,
                    tooltip=f"<span style='font-size:1.5em;'><strong>DER</strong>: {model.DERs[model.DERs['connected_bus'] == to_node]['name'].item()} </span>",
                ).add_to(distribution_map)
            else:
        ...
        if round(model.xij[edge_index].value) == 1:
            if data["is_switch"]:
                ...
                if powerflow[edge_index] > 0:
                    # normal line
                    colorcheck = "black"
                    weightcheck = (widthmax - widthmin) * (
                        (powerflow[edge_index] - min_flow) / (max_flow - min_flow)
                    ) + widthmin
                else:
                    colorcheck = "gray"
...
def plot_network_on_map(
    ...
    for node, data in network_tree.nodes(data=True):
        ...
        CircleMarker(
            ...
            radius=3,
            color="blue",
            tooltip=f"<span style='font-size:1.5em;'>{node}</span>",
        ).add_to(distribution_map)
    ...
    for from_node, to_node, data in network_graph.edges(data=True):
        ...
        PolyLine(
            ...
            opacity=1,
            tooltip=f"<span style='font-size:1.5em;'>{data['element'] if not data['is_switch'] else 'switch'}</span>",
        ).add_to(distribution_map)
...
def plot_cartesian_network(
    cartesian_tree: nx.DiGraph,
    ...
    nodes_df = pd.DataFrame(
        [(node, data["lat"], data["lon"]) for node, data in cartesian_tree.nodes(data=True)],
        columns=["node", "X", "Y"],
    )
    ...
    edges_df = pd.DataFrame(
        [
            (from_node, to_node, data["element"], data["is_switch"])
            for from_node, to_node, data in cartesian_graph.edges(data=True)
        ],
        columns=["source", "target", "element", "is_switch"],
    )
    ...
    fig.update_layout(xaxis=dict(title="X"), yaxis=dict(title="Y"), showlegend=False)
