...
if TYPE_CHECKING:
    from ldrestoration.dssparser import NetworkHandler, TransformerHandler
...
class LoadHandler:
    ...
    Args:
        dss_instance (AltDSS): altdss instance
    ...
    def __init__(
        ...
        If `include_secondary_network=False`, all the secondary loads are referred back to their primary.
        This requires the network_handler and transformer_handler instances to be passed as arguments to the LoadHandler instance.
        ...
        if bus_names is None:
            self.bus_names = self.dss_instance.BusNames()
        else:
            self.bus_names = bus_names

    def __load_input_validator(self) -> None:
        """Validate the inputs provided to the LoadHandler instance.

        Raises:
            NotImplementedError: If required handlers are not provided when secondary network is not included.
        """

        if not self.include_secondary_network and (not self.transformer_handler and not self.network_handler):
            logger.warning(
                "When include_secondary_network=False, you need to provide NetworkHandler() and TransformerHandler as arguments to LoadHandler()"
            )
            msg = "To refer the loads to primary, both NetworkHandler and TransformerHandler are required."
            raise NotImplementedError(msg)

    def get_loads(self) -> pd.DataFrame:
        """Get the load information for the distribution grid.

        Returns:
            pd.DataFrame: DataFrame containing load information for each bus and phase.
        """
        ...
        while loads_flag:
            ...
            for buses in connected_buses:
                ...
                if (len(bus_split) == 4) or (len(bus_split) == 1):
                    ...
                    connected_bus, connected_phase_secondary = (
                        bus_split[0],
                        bus_split[1:],
                    )
                    bus_index = self.bus_names_to_index_map[connected_bus]
                    ...
                    Q_values = nonzero_power[
                        1::2
                    ]  # Extract Q values (every alternate element starting from the second)
    ...
    def get_primary_referred_loads(self) -> pd.DataFrame:
        ...
        for xfrmr_secondary_node, primary_phase in split_phase_primary.items():
            ...
            for xfrmr_downstream_node in xfrmr_downstream_nodes:
                ...
                primary_loads_df.loc[primary_bus_index, f"P{primary_phase[0]}"] += (
                    primary_loads_df["P1"][downstream_node_index] + primary_loads_df["P2"][downstream_node_index]
                )
                primary_loads_df.loc[primary_bus_index, f"Q{primary_phase[0]}"] += (
                    primary_loads_df["Q1"][downstream_node_index] + primary_loads_df["Q2"][downstream_node_index]
                )
